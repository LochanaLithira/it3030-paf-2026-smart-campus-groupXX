...
-- ================================================================
-- 1. MEMBER 4: AUTHENTICATION & ROLES
-- ================================================================

-- Core User Table (Supports OAuth 2.0 / Google sign-in)
CREATE TABLE IF NOT EXISTS users (
    ...
    oauth_provider_id VARCHAR(255),                -- provider-specific subject id
...
CREATE TABLE IF NOT EXISTS roles (
    ...
    permissions TEXT[] NOT NULL DEFAULT '{}'    -- e.g. {'MANAGE_USERS','APPROVE_BOOKINGS','VIEW_REPORTS'}
);

-- Many-to-many mapping for RBAC
...
-- ================================================================
-- 2. MEMBER 1: FACILITIES & ASSETS
-- ================================================================

-- Physical locations (buildings + floors)
...
-- Campus resources (rooms, labs, equipment …)
...
-- Weekly recurring availability windows per resource
...
-- Optional tags / attributes for filtering (e.g. "projector", "whiteboard")
...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    -- Prevent overlapping APPROVED bookings on the same resource + date
...
-- Recurring booking groups (optional; each occurrence is still a row in bookings)
CREATE TABLE IF NOT EXISTS recurring_booking_groups (
    group_id        UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id         UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    resource_id     UUID NOT NULL REFERENCES resources(resource_id) ON DELETE CASCADE,
    recurrence_rule VARCHAR(255) NOT NULL,  -- iCal RRULE string e.g. "FREQ=WEEKLY;BYDAY=MO,WE"
...
-- Link individual bookings back to a recurring group
ALTER TABLE bookings
    ADD COLUMN recurring_group_id UUID REFERENCES recurring_booking_groups(group_id) ON DELETE SET NULL;


-- ================================================================
-- 4. MEMBER 3: MAINTENANCE & TICKETING
-- ================================================================
...
-- Ticket status history for auditing
...
-- ================================================================
-- 5. MEMBER 4: NOTIFICATION SYSTEM
-- ================================================================
...
CREATE TABLE IF NOT EXISTS notifications (
    ...
    related_entity_id UUID,              -- generic FK to booking / ticket / etc.
    created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()
);


-- ================================================================
-- 6. INDEXES  (beyond PKs & UNIQUEs which are auto-indexed)
-- ================================================================
